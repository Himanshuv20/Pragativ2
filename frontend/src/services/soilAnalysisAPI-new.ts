...
export const soilAnalysisAPI = {
   ...
   */
  async analyzeSoil(params: { lat: number; lon: number; date?: string }) {
    try {
   ...
   */
  async geocodeLocation(city: string) {
    try {
   ...
   */
  async getSentinelData(params: { lat: number; lon: number; start?: string; end?: string }) {
    try {
